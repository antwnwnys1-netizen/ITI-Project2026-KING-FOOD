...
async function connectMongo() {
  try {
    ...
    } catch (e) { /* migration non-critical */ }
...
app.post('/api/user/cart', authOnly, async (req, res) => {
  try {
    ...
    if (product.stockQuantity <= 0) return res.status(400).json({ message: 'Out of stock' });
    const idx = c.items.findIndex(i => String(i.product) === String(productId));
    const existingQty = idx >= 0 ? c.items[idx].qty : 0;
    if (existingQty + qty > product.stockQuantity) return res.status(400).json({ message: 'Not enough stock' });
...
app.put('/api/user/cart/qty', authOnly, async (req, res) => {
  try {
    ...
    if (it) { const stock = it.product.stockQuantity || 0; it.qty = Math.max(1, Math.min(stock, it.qty + Number(delta || 0))); }
    await c.save();
...
const AI_EYEGPT_KEY     = process.env.AI_EYEGPT_KEY;
const AI_OPENROUTER_KEY = process.env.AI_OPENROUTER_KEY;
const AI_ATXP_KEY       = process.env.AI_ATXP_KEY;
const AI_ATXP_ACCOUNT_ID = process.env.AI_ATXP_ACCOUNT_ID;
...
const AI_PROVIDERS = [
  ...
  },
  {
    ...
    key: AI_ATXP_KEY,
    headers: k => ({ 'Authorization': `Bearer ${k}`, 'Content-Type': 'application/json', 'x-account-id': AI_ATXP_ACCOUNT_ID })
  },
  {
    name: 'atxp',
    model: 'Claude-Opus-4.7',
...
function logFailover(event) {
  ...
  failoverLog.push(entry);
  if (failoverLog.length > 200) failoverLog.shift();
  console.error(`[AI-FAILOVER] ${event.from || 'start'} -> ${event.to || 'END'} | model: ${event.model || 'N/A'} | reason: ${event.reason || 'N/A'} | attempt: ${event.attempt || 0}`);
}
...
async function callAI(providers, body, attempt = 0) {
  if (attempt >= providers.length) {
    logFailover({ from: providers[providers.length - 1]?.name || 'unknown', to: 'NONE', model: 'ALL_EXHAUSTED', reason: 'All providers exhausted', attempt });
    return null;
  }
  const p = providers[attempt];
  const nextName = attempt + 1 < providers.length ? providers[attempt + 1].name : 'NONE';
  try {
    ...
    if (!resp.ok) {
      ...
      logFailover({ from: p.name, to: nextName, model: p.model, reason: `HTTP ${resp.status}: ${t.slice(0, 200)}`, attempt: attempt + 1 });
    ...
    if (!content) {
      logFailover({ from: p.name, to: nextName, model: p.model, reason: 'Empty response content', attempt: attempt + 1 });
      await new Promise(r => setTimeout(r, 500));
      return callAI(providers, body, attempt + 1);
    }
    return content;
    ...
    const isNetwork = e.type === 'ERR_CONNECTION_REFUSED' || e.message?.includes('ENOTFOUND') || e.message?.includes('ECONNREFUSED') || e.message?.includes('ETIMEDOUT') || e.message?.includes('ECONNRESET') || e.message?.includes('network') || e.message?.includes('Network');
    const reason = isAbort ? 'Timeout (30s)' : isNetwork ? 'Connection/Network error' : e.message || 'Unknown error';
    logFailover({ from: p.name, to: nextName, model: p.model, reason, attempt: attempt + 1 });
    await new Promise(r => setTimeout(r, isNetwork ? 1000 : 500));
    return callAI(providers, body, attempt + 1);
  }
}
...
📅 Bookings: ${context.bookingInfo}`;
    ...
    let reply = await callAI(AI_PROVIDERS, aiBody);
    if (!reply) {
      console.error('[AI-FAILOVER] All models exhausted — returning graceful fallback message');
      reply = 'I apologize, but I\'m having trouble connecting to my AI services right now. Please try again in a moment, or contact our support team for immediate assistance.';
    }
      ...
      if (!existingOrder) {
        ...
        for (const item of transaction.items) {
          if (item.product) {
            ...
            if (prod) {
              prod.stockQuantity = Math.max(0, (prod.stockQuantity || 0) - item.qty);
